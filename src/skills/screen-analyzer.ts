...
const PURPOSE_PROMPTS: Record<string, string> = {
  tutorial:     "「〜します」「〜をクリックします」のような丁寧な操作解説調",
  presentation: "「こちらのスライドでは〜」「ポイントは〜」のようなプレゼン解説調",
  service:      "「このサービスでは〜」「特徴として〜」のようなサービス紹介調",
};
...
async function callWithRetry<T>(fn: () => Promise<T>, retries: number = 3): Promise<T> {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      log("screen-analyzer", `リトライ ${attempt}/${retries}... (${wait}ms待機)`);
...
async function analyzeBatch(
  ...
  const style = PURPOSE_PROMPTS[purpose] ?? PURPOSE_PROMPTS["tutorial"];
...
2. ナレーション: 各セグメントの操作を視聴者に説明する日本語ナレーション（上記スタイルに従う）
...
export async function run(config: PipelineConfig, plan: ProductionPlan): Promise<StepResult> {
  ...
  try {
    ...
    log("screen-analyzer", `${allFrames.length} フレームを解析します (purpose: ${plan.purpose})`);

    const genAI = new GoogleGenerativeAI(apiKey);
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash" });

    const batches = createBatches(allFrames, config.maxBatchSize, config.batchOverlap);
    log("screen-analyzer", `${batches.length} バッチに分割`);
    ...
    for (let i = 0; i < batches.length; i++) {
      ...
      log("screen-analyzer", `バッチ ${i + 1}/${batches.length} (${batch.length}フレーム) 解析中...`);

      const result = await analyzeBatch(
        model, batch, i, startFrameGlobal, metadata.fps, previousContext, plan.purpose
      );
      ...
      log("screen-analyzer", `バッチ ${i + 1}: ${result.segments.length}セグメント, ${result.cuts.length}カット区間`);
    }
    ...
    log("screen-analyzer", `解析完了: ${allSegments.length}セグメント, ${allCuts.length}カット区間`);

    const durationMs = Date.now() - start;
    logStepDone("screen-analyzer", durationMs);
    return { step: "analyze", success: true, durationMs, outputPath: scriptPath };

  } catch (error) {
    logError("screen-analyzer", error);
