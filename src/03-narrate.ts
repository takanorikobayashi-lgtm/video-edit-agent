...
async function synthesizeWithGemini(
  ...
  // mimeTypeからサンプルレートを取得 (例: audio/L16;codec=pcm;rate=24000)
...
export async function narrate(config: PipelineConfig): Promise<StepResult> {
  const start = Date.now();
  logStepStart("narrate");
  ...
  try {
    ...
    const activeSegments = script.segments.filter((s) => s.keep);
    log("narrate", `${activeSegments.length} セグメントの音声を生成します (Gemini TTS)`);
      ...
      return async (): Promise<AudioEntry> => {
        ...
        for (let attempt = 1; attempt <= config.maxRetries; attempt++) {
          try {
            ...
            log("narrate", `リトライ ${attempt}/${config.maxRetries}... (${wait}ms待機)`);
            await new Promise((r) => setTimeout(r, wait));
          }
        }

        const duration = getAudioDuration(filePath);
        log("narrate", `✓ セグメント ${segment.id}: ${duration.toFixed(1)}秒`);
        totalCharacters += segment.narration.length;
    ...
    log("narrate", `音声生成完了: 合計 ${manifest.totalDuration.toFixed(1)}秒`);

    const durationMs = Date.now() - start;
    logStepDone("narrate", durationMs);
    return { step: "narrate", success: true, durationMs, outputPath: audioDir };

  } catch (error) {
    logError("narrate", error);
